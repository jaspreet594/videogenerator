...
import { generateSceneImage, generateStoryboard } from './services/geminiService';
import { getAudioContext } from './services/audioUtils';
import { SceneCard } from './components/SceneCard';
import { Player } from './components/Player';
import { Sparkles, Wand2, AlertTriangle, Upload, Music, Type, FileText, Clock, ArrowDown, Zap, PlayCircle, RotateCcw } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handleSmartGenerate = async () => {
    ...
    try {
      const segments = await generateStoryboard(rawStoryInput);
  ...
  const generateVisuals = async (currentProject: ProjectData) => {
    ...
    for (let i = 0; i < scenes.length; i++) {
      try {
        ...
        const imageUrl = await generateSceneImage(scenes[i].prompt);
  ...
  return (
    <div className="min-h-screen flex flex-col h-screen bg-gray-950 text-gray-100">
      {/* Header */}
      <header className="h-14 bg-gray-900 border-b border-gray-800 flex items-center justify-between px-6 shrink-0">
        <div className="flex items-center gap-2" onClick={() => setProject(null)}>
